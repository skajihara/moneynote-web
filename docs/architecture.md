...
**主要機能:**
- マルチアカウント・マルチ帳簿（帳簿共有・権限管理）
- 収支明細管理（カレンダー・一覧・検索）
- 分析レポート（月次・年間・カテゴリ別）
- 予算管理・固定費管理
- CSV エクスポート・インポート
- AI 支出アドバイス（Claude API）
...
```mermaid
graph TB
    Browser["ブラウザ (HTTPS :443 / HTTP :80)"]
...
| Springdoc OpenAPI | 2.8.5 | Swagger UI 自動生成 |
| JUnit5 | — | 単体・結合テスト |
...
サブパネルはドラッグで幅変更可能（最小 280px・最大 画面幅の 60%）。
...
| OWNER 管理 | OWNER は `ledger_permissions` テーブルに保存しない（`ledger.owner_user_id` で管理） |
...
└── domain
    ...
    ├── ledgerpermission # 帳簿権限（Controller・Service・Entity・Repository）
...
│       ├── dashboard
│       ├── ledgers/[ledgerId]
│       │   ├── transactions     # 収支明細（カレンダー・一覧）
│       │   ├── search           # 明細検索
│       │   ├── reports          # 分析レポート
│       │   ├── categories       # カテゴリ別分析
│       │   ├── budget           # 予算管理
│       │   └── ai               # AI 支出アドバイス
│       └── settings             # 設定（アカウント・帳簿管理）
├── components
│   ├── budget
│   ├── charts
│   ├── csv
...
| トークン偽装 | JWT `type=ACCESS` クレーム検証（リフレッシュトークンの Bearer 悪用防止） |

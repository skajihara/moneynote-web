...
# CSRの生成
openssl req -new -key key.pem -out csr.pem -subj "/CN=moneynote-env1"

# 自己署名証明書の生成（有効期限: 825日）
openssl x509 -req -days 825 -in csr.pem -signkey key.pem -out cert.pem
...
> ⚠️ ACMのコンソール上でインポート済み証明書はALB作成画面のドロップダウンに表示されない仕様のため、ALB作成後にリスナー追加画面からARNを直接入力して設定する。
...
- パブリックIPv4アドレスの自動割り当て: OFF（全サブネット共通）
```
...
```
CloudShellを開いて以下を順番に実行する

openssl genrsa -out key.pem 2048
openssl req -new -key key.pem -out csr.pem -subj "/CN=moneynote-env1"
openssl x509 -req -days 825 -in csr.pem -signkey key.pem -out cert.pem
aws acm import-certificate --certificate fileb://cert.pem --private-key fileb://key.pem --region ap-northeast-1
...
**手順14: Secrets Managerへのシークレット登録（CloudShellで実行）**
```
Secrets Manager → シークレットを保存
以下の4つを登録する（タイプ: その他のシークレット）

- moneynote/env1/db-password
- moneynote/env1/jwt-secret
- moneynote/env1/redis-password
- moneynote/env1/claude-api-key
```
...
**手順16: 本番用設定ファイルの作成**
```
docker-compose.env1.yml を作成する
application-env2.yml を作成する
機密情報はSecrets Manager経由で取得する（ハードコード禁止）
```
...
Step 17（4.8）の手順を環境2用のリソース名・設定・CIDRで繰り返す。

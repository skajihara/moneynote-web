# MoneyNote Web — AWSデプロイ設計書（Step 17〜22・最終版）

**作成日:** 2026年5月  
**バージョン:** 1.6（最終）  
**区分:** 個人開発  
**状態:** ⚠️ **AWSインフラ削除済み（2026年5月）。本書はポートフォリオ用記録として保存。**
...
11. [AWSインフラ削除記録（2026年5月）](#11-awsインフラ削除記録2026年5月)
...
| 最終バージョン | v1.6.0（リポジトリ公開・監査完了） |
| 対象ドキュメント | Step 17〜22 AWSデプロイ計画・最終版 |
...
| v1.6.0 | — | リポジトリ公開前監査・コード品質改善・パブリック公開 | ポートフォリオ公開 ✅ |

> **[2026年5月] v1.6.0 リリース後、ポートフォリオ公開にあたりコスト最適化のため全AWSリソースを削除。**  
> 以降はローカルDocker Compose環境のみ稼働。CI/CDはテストジョブのみ有効。
...
| ALB | ALB-ka-moneynote-01 | ALB-ka-moneynote-02 | **名前は大文字「ALB-」で始まる（AWS CLI --names は大文字小文字区別）** |
...
| SES | 送信数に応じた従量課金 | $0〜 | $0〜 | Step 22で追加 |
...
| secretsmanager:DescribeSecret | シークレット情報の確認のため |
...
| elasticloadbalancing:DescribeLoadBalancers | secrets-fetch.sh でALB DNS名を動的取得するため（Step 22で追加） |
...
### 4.7 secrets-fetch.sh の仕様（最終版・Step 22完了時点）
...
ALB_DNS=（aws elbv2 describe-load-balancers --names "ALB-ka-moneynote-0x" で動的取得）
FRONTEND_URL=https://${ALB_DNS}

# 固定値（機密情報ではない）
ECR_REGISTRY=（aws sts get-caller-identityでAWSアカウントIDを取得して動的生成）
DB_PORT=5432 / DB_NAME=moneynote / DB_USERNAME=moneynote / REDIS_PORT=6379

# SES SMTP（Step 22で追加）
MAIL_HOST=email-smtp.ap-northeast-1.amazonaws.com
MAIL_PORT=587
MAIL_USERNAME=（moneynote/ses-smtp の username キーを取得）
MAIL_PASSWORD=（moneynote/ses-smtp の password キーを取得）
MAIL_FROM=（ADMIN_EMAILと同じ値）
```

> ⚠️ **ALB名の大文字小文字:** `aws elbv2 describe-load-balancers --names` は**大文字小文字を区別する**。  
> ALBの実際の名前は `ALB-ka-moneynote-01`（大文字「ALB」）。`alb-ka-moneynote-01`（小文字）では`LoadBalancerNotFound`エラーになる。

> ⚠️ Secrets Managerから取得した値はJSON形式（例: `{"db-password":"xxxx"}`）で返るため
> Python等でパースして値のみを取り出す必要がある。  
> `get_secret()`: JSON値が1キーの場合に使用（`list(json.values())[0]`）  
> `get_secret_key("シークレット名", "キー名")`: 複数キーのJSONから特定キーを取得（SES SMTP等）
...
① IAM_P_ka_moneynote_01（ポリシー）
  ...
  - CloudWatch Logs: CreateLogGroup・CreateLogStream・
    ...
    リソース: すべて
  - ELB: elasticloadbalancing:DescribeLoadBalancers（Step 22で追加）
    リソース: すべて
...
GitHub Actions
  ...
  ├── build-and-push（Dockerイメージ → ECR）  ※インフラ削除後は無効化
  └── deploy（SSM Send Command → EC2がECRからpull → docker compose up）  ※インフラ削除後は無効化
```

> **[2026年5月] AWSインフラ削除後:** `build-and-push` および `deploy` ジョブは `.github/workflows/ci-cd.yml` でコメントアウト済み。  
> `test-backend` および `test-frontend` のみ有効。インフラ再構築時はコメントアウトを解除する。
...
> ✅ **GitHub Secretsのセキュリティ:** パブリックリポジトリであってもGitHub SecretsはリポジトリオーナーとGitHub Actionsのみがアクセス可能。フォーク先のリポジトリからはアクセス不可。インフラ削除後もSecretsを保持していても安全。
...
**Step 22 完了済み（v1.5.0・2026年5月）。**

### 8.1 SES（Simple Email Service）

| 項目 | 内容 |
|---|---|
| 用途 | 予算超過通知・アカウント登録確認メール・パスワードリセット・メールアドレス変更確認・アカウント削除通知 |
| 送信元アドレス | SESで検証済みのアドレスを使用（`moneynote/ses-from-address` から取得） |
| SMTPエンドポイント | `email-smtp.ap-northeast-1.amazonaws.com:587`（STARTTLS） |
| SMTP認証情報 | `moneynote/ses-smtp`（username・password）で管理 |
| 送信制限 | **SESサンドボックスモードでは検証済みアドレス以外に送信不可。本番運用にはサンドボックス解除申請が必要** |
| Spring Boot設定 | application-env1/2.yml にSMTP認証・STARTTLS設定を追加 |
| ヘルスチェック | `management.health.mail.enabled: false` のまま運用（SES接続確認後に有効化） |

> ⚠️ **ALL.5対応:** SESのサンドボックス解除はアカウント全体に影響するため、アカウント管理者の許可を得てから申請する。

### 8.2 Secrets Manager 最終構成
...
| moneynote/env1/admin-email | システム管理者メールアドレス | 問い合わせメール受信先・MAIL_FROM |
| moneynote/ses-smtp | SES SMTP認証情報（username・password）| メール送信機能（env1・env2共通） |
| moneynote/env2/\* | env2用（db-password・jwt-secret・redis-password・claude-api-key・admin-email） | 環境2の各機能 |

> ✅ **`moneynote/ses-smtp`は env1・env2 で共通のシークレット。** `get_secret_key` 関数でキー名を指定して取得する（`get_secret` では複数キーのため取得不可）。

### 8.3 Step 22 で実施した主な変更

| 変更対象 | 内容 |
|---|---|
| `scripts/secrets-fetch.sh` | `get_secret_key` 関数を追加。`moneynote/ses-smtp`（username/password）を取得。ALB DNS名を `aws elbv2 describe-load-balancers` で動的取得（**名前は大文字「ALB-ka-moneynote-0x」**）。`MAIL_HOST=email-smtp.ap-northeast-1.amazonaws.com` に変更 |
| `application-env1/2.yml` | SMTP認証（username/password/starttls）と `app.mail.from` を追加 |
...
| パスワードリセット | 再申請時に旧トークンを無効化（Redis リバースマッピングパターン） |
| メールアドレス変更 | 確認フロー追加（変更後アドレスに確認メール送信・30分TTL・旧リンク無効化） |
| アカウント削除通知 | キャンセルリンクの有効期限表示を日時形式（例: 2026/05/27 23:59:59 JST）に変更 |
| IAM_P_ka_moneynote_01 | `elasticloadbalancing:DescribeLoadBalancers` を追加 |
...
- **`org.hibernate.orm.jdbc.bind: TRACE` はSQL bind値（パスワードハッシュ等）をログ出力するため本番環境では設定禁止**（`application-env1/2.yml` では削除済み）
...
| — | v1.6.0 | リポジトリ公開前監査・パブリック公開 ✅ | — | AWSインフラ削除・ローカルのみ稼働 |

---

## 11. AWSインフラ削除記録（2026年5月）

### 11.1 削除の背景

v1.6.0リリース（リポジトリ公開前監査・コード品質改善完了）後、ポートフォリオ公開にあたりコスト最適化のため全AWSリソースを削除した。インフラ設計・構築・CI/CDパイプラインの実装内容は本書に記録として保存する。

### 11.2 削除したリソース一覧

削除は依存関係を考慮して以下の順序で実施した。

| 削除順 | リソース種別 | リソース名 | 備考 |
|---|---|---|---|
| 1 | EC2インスタンス | EC2_ka_moneynote_01・EC2_ka_moneynote_02 | アプリケーション層 |
| 2 | ALB | ALB-ka-moneynote-01・ALB-ka-moneynote-02 | ターゲットグループより先 |
| 3 | ターゲットグループ | TGT-ka-moneynote-01・TGT-ka-moneynote-02 | |
| 4 | RDS | RDS-ka-moneynote-01・RDS-ka-moneynote-02 | スナップショット取得後に削除 |
| 5 | ElastiCache | ELC-ka-moneynote-01・ELC-ka-moneynote-02 | |
| 6 | NAT Gateway | NGW_ka_moneynote_01 | Elastic IPも解放 |
| 7 | ECRリポジトリ | ecr-ka-moneynote-backend・ecr-ka-moneynote-frontend | |
| 8 | セキュリティグループ | SEG_ka_moneynote_01〜12 | EC2/RDS等が削除済みであること確認後 |
| 9 | サブネット | SBN_ka_moneynote_01〜12 | |
| 10 | ルートテーブル | RTB_ka_moneynote_01〜12 | メインルートテーブル以外 |
| 11 | Internet Gateway | IGW_ka_moneynote_01 | VPCからデタッチ後に削除 |
| 12 | VPC | VPC_ka_moneynote_01 | 全リソース削除後 |
| 13 | Secrets Manager | moneynote/\* | 全シークレット |
| 14 | SES設定 | 送信元アドレス検証・SMTP認証情報 | |
| 15 | AMI・スナップショット | AMI_ka_moneynote_\* | バックアップ用AMIも削除 |
| 16 | S3 | S3_ka_moneynote_01 | コンテンツ削除後にバケット削除 |
| — | IAMロール・ポリシー | IAM_R_01・02 / IAM_P_01・02 | **残存（グローバルリソース）** |
| — | ACM証明書 | 自己署名証明書 | **残存（課金なし）** |
| — | GitHub Secrets | AWS_ROLE_ARN 等 | **残存（パブリックリポジトリでも安全）** |

### 11.3 削除後のCI/CD状態

`.github/workflows/ci-cd.yml` の `build-and-push` および `deploy` ジョブをコメントアウトした。  
テストジョブ（`test-backend`・`test-frontend`）は引き続き有効。

```
現在有効なジョブ:
  ✅ test-backend  （./gradlew test・JUnit5 + Testcontainers）
  ✅ test-frontend （npm test・Jest + RTL）

無効化（コメントアウト）:
  ❌ build-and-push（ECRへのDockerイメージプッシュ）
  ❌ deploy        （SSM Run CommandでのEC2デプロイ）
```

インフラ再構築時は `ci-cd.yml` のコメントアウトを解除して使用する。

### 11.4 ローカル環境での動作確認

AWSインフラ削除後は以下のローカル環境で動作確認可能。
